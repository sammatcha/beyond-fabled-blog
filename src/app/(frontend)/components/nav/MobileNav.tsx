   ...
   const  filteredLinks = links.filter(link => link.name !== "Home" )
    return(
        ...
        <nav className="block lg:hidden relative z-20 ">
                            ...
                            {/* x button */}
                        <button onClick={handleOnClick} className="absolute z-50 mt-3 left-0 px-2 cursor-pointer">
                            {isOpen ? (
                                <CircleX className=" w-8 text-black "  />  
                            ) : (
                                    
                                < Menu className="w-10 text-black" />
                            )
                        }
                        ...
                        <Link key={id} href={link.link} className="flex flex-col  ml-1 mt-10 text-cuteBlue font-karla font-bold hover:text-[#93CED6]" > 
                        {link.name}
                        </Link>
